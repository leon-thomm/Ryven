...
class Rand_Add_Node(NodeBase):
    title = 'RAND_add'
    type_ = 'ssl'
    doc = """Mix string into the OpenSSL PRNG state.
...
class Rand_Bytes_Node(NodeBase):
    title = 'RAND_bytes'
    type_ = 'ssl'
    doc = """Generate n cryptographically strong pseudo-random bytes."""
    init_inputs = [
        NodeInputBP(label='n'),
    ]
    init_outputs = [
...
class Rand_Pseudo_Bytes_Node(NodeBase):
    ...
    type_ = 'ssl'
    doc = """Generate n pseudo-random bytes.
...
class Rand_Status_Node(NodeBase):
    title = 'RAND_status'
    type_ = 'ssl'
    doc = """Returns 1 if the OpenSSL PRNG has been seeded with enough data and 0 if not.
...
class _Create_Stdlib_Context_Node(NodeBase):
    ...
    type_ = 'ssl'
    doc = """Create a SSLContext object for Python stdlib modules
...
class _Dnsname_Match_Node(NodeBase):
    ...
    type_ = 'ssl'
    doc = """Matching according to RFC 6125, section 6.4.3
...
class _Inet_Paton_Node(NodeBase):
    title = '_inet_paton'
    type_ = 'ssl'
    doc = """Try to convert an IP address to packed binary form
...
class _Ipaddress_Match_Node(NodeBase):
    ...
    type_ = 'ssl'
    doc = """Exact matching of IP addresses.
...
class _Nid2Obj_Node(NodeBase):
    title = '_nid2obj'
    type_ = 'ssl'
    doc = """Lookup NID, short name, long name and OID of an ASN1_OBJECT by NID."""
    init_inputs = [
...
class _Sslcopydoc_Node(NodeBase):
    title = '_sslcopydoc'
    type_ = 'ssl'
    doc = """Copy docstring from SSLObject to SSLSocket"""
    init_inputs = [
...
class _Txt2Obj_Node(NodeBase):
    title = '_txt2obj'
    type_ = 'ssl'
    doc = """Lookup NID, short name, long name and OID of an ASN1_OBJECT.
...
class Enum_Certificates_Node(NodeBase):
    ...
    type_ = 'ssl'
    doc = """Retrieve certificates from Windows' cert store.
...
class Enum_Crls_Node(NodeBase):
    title = 'enum_crls'
    type_ = 'ssl'
    doc = """Retrieve CRLs from Windows' cert store.
