...
A package is identified by it's name. Packages with the same names should never exist. If you want to edit an existing package (changing, adding, removing nodes), you need to start the NodeManager, import the package, do the changes that you want to perform and save it again. You can and should choose the original package directory in the end. None of your content will be deleted or edited. Source code files get only created if they do not already exist (see next section) and all your custom files and folders you have put somwhere into the packages directory stay as they are. It is important, that you keep one main name for your package. If you create different versions of it and use different versions in different pyScript projects, you can only use these versions in the corresponding project. So if you want to edit an existing package, save it as a new package first, test it and when everything works, overwrite the original package by just exporting it to the original package folder from the NodeManager.
